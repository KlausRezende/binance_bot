...
# -*- coding: utf-8 -*-
"""
VENDEDOR SIMPLES DE BITCOIN
Script para vender Bitcoin com opções pré-definidas
"""
...
def vender_bitcoin_simples():
    print("💰 VENDEDOR SIMPLES DE BITCOIN")
    print("=" * 40)
    ...
    try:
        ...
        try:
            ...
            if opcao == "1" and total_value * 0.20 >= min_value:
                percentage = 0.20
                quantity_to_sell = btc_balance * percentage
            ...
            print(f"   Quantidade: {quantity_to_sell:.8f} BTC")
            ...
            if confirmacao in ['s', 'sim', 'y', 'yes']:
                # Executa a venda
                print(f"\n🔄 Executando venda de {quantity_to_sell:.8f} BTC...")
                
                # Formata a quantidade corretamente para a API (incremento de 0.00001000)
                step_size = 0.00001000
                quantity_formatted = round(quantity_to_sell / step_size) * step_size
                quantity_formatted = f"{quantity_formatted:.8f}".rstrip('0').rstrip('.')
                ...
                print("✅ Log salvo em: C:\\Users\\klaus\\Documents\\Coisas\\Scripts\\bot_binance\\logs\\trading_bot.log")
                
                # Mostra o resultado
                print(f"\n🎉 VENDA CONCLUÍDA!")
                print(f"   Quantidade vendida: {float(order['executedQty']):.8f} BTC")
                ...
                remaining_btc = btc_balance - float(order['executedQty'])
                if remaining_btc > 0:
                    print(f"   Bitcoin restante: {remaining_btc:.8f} BTC")
